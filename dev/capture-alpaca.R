...
#
# ============================================================================
# SHAPE-VERIFICATION TOOL ONLY. CAPTURES NEVER ENTER THE REPO.
# ============================================================================
#
# Per the connector constitution (CONNECTOR-CONVENTIONS.md I.3.6), the committed
# fixtures under tests/testthat/fixtures/ are AUTHORED-SYNTHETIC: hand-written,
# obviously-fake values (round numbers, TEST/EXAMPLE ids), never captured from
# the live API even scrubbed. This harness exists solely to VERIFY that those
# synthetic fixtures stay shape-faithful to the live wire format — it is NOT a
# fixture generator, and its output must NEVER be committed or copied into a
# fixture.
#
...
# tests/testthat/fixtures/ to confirm the fixtures faithfully mirror the live
# wire SHAPES (field names, nesting, nullability) — never their values.
#
...
# written ONLY under local/raw-data/alpaca/ which is git-ignored, and MUST NOT
# be committed to the repository under any circumstances.
#
...
if (!nzchar(trading_raw)) {
  trading_raw <- Sys.getenv("ALPACA_API_ENDPOINT")
}
if (!nzchar(trading_raw)) {
  trading_raw <- "https://paper-api.alpaca.markets"
}
...
capture("portfolio_history", TRADING_HOST, "/v2/account/portfolio/history", list(period = "1M", timeframe = "1D"))
capture("positions", TRADING_HOST, "/v2/positions")
orders_res <- capture("orders_list", TRADING_HOST, "/v2/orders", list(status = "all", limit = 50, direction = "desc"))
capture("assets", TRADING_HOST, "/v2/assets", list(status = "active", asset_class = "us_equity"))
capture("asset", TRADING_HOST, "/v2/assets/AAPL")
wl_res <- capture("watchlists", TRADING_HOST, "/v2/watchlists")
capture("calendar", TRADING_HOST, "/v2/calendar", list(start = win_start, end = win_end))
...
if (!is.null(oc_symbol)) {
  capture("option_contract", TRADING_HOST, paste0("/v2/options/contracts/", oc_symbol))
}
...
capture(
  ...
  DATA_HOST,
  "/v2/stocks/bars",
  list(symbols = "AAPL,MSFT", timeframe = "1Day", start = bars_start, limit = 2, feed = "iex")
)
capture("latest_bar", DATA_HOST, "/v2/stocks/AAPL/bars/latest", list(feed = "iex"))
capture("latest_bars_multi", DATA_HOST, "/v2/stocks/bars/latest", list(symbols = "AAPL,MSFT", feed = "iex"))
capture("trade", DATA_HOST, "/v2/stocks/AAPL/trades/latest", list(feed = "iex"))
capture("latest_trades_multi", DATA_HOST, "/v2/stocks/trades/latest", list(symbols = "AAPL,MSFT", feed = "iex"))
capture("quote", DATA_HOST, "/v2/stocks/AAPL/quotes/latest", list(feed = "iex"))
capture("latest_quotes_multi", DATA_HOST, "/v2/stocks/quotes/latest", list(symbols = "AAPL,MSFT", feed = "iex"))
capture("snapshot", DATA_HOST, "/v2/stocks/AAPL/snapshot", list(feed = "iex"))
capture("snapshots_multi", DATA_HOST, "/v2/stocks/snapshots", list(symbols = "AAPL,MSFT", feed = "iex"))

# Screener
capture("most_actives", DATA_HOST, "/v1beta1/screener/stocks/most-actives", list(top = 5))
...
# Crypto orderbook
capture("crypto_orderbook", DATA_HOST, "/v1beta3/crypto/us/latest/orderbooks", list(symbols = "BTC/USD"))
...
capture("option_chain", DATA_HOST, "/v1beta1/options/snapshots/AAPL", list(feed = "indicative", limit = 10))
if (!is.null(oc_symbol)) {
  capture(
