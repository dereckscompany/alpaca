# Asynchronous Usage with alpaca

Every R6 class in `alpaca` accepts an `async = TRUE` flag at
construction. When enabled, all methods return
[`promises::promise`](https://rstudio.github.io/promises/reference/promise.html)
objects instead of direct values. This vignette shows how to consume
those promises with
[`coro::async`](https://coro.r-lib.org/reference/async.html)/`await` and
[`later::run_now`](https://later.r-lib.org/reference/run_now.html).

## Why Async?

Synchronous HTTP blocks the R session while waiting for a reply.
Asynchronous mode lets you fire off multiple requests and process
results as they arrive — useful for bots that poll several endpoints or
place orders in parallel.

## Event Loop

> **Event loop**: R does not have a built-in event loop like Node.js or
> Python’s `asyncio`. Promises only resolve when the event loop ticks
> via
> [`later::run_now()`](https://later.r-lib.org/reference/run_now.html).
> In scripts and vignettes, drain the loop with
> `while (!loop_empty()) run_now()`. In **Shiny** apps the event loop
> runs automatically.

## Basic Async with coro

The `coro` package provides
[`async()`](https://coro.r-lib.org/reference/async.html) and
[`await()`](https://coro.r-lib.org/reference/async.html) for writing
async code that reads like synchronous code:

``` r

library(alpaca)
library(coro)
library(later)

md <- AlpacaMarketData$new(async = TRUE)

main <- async(function() {
  # These run concurrently
  bars_promise <- md$get_bars(
    symbol = "AAPL",
    timeframe = "1Day",
    start = "2024-01-01",
    end = "2024-01-31"
  )
  clock_promise <- md$get_clock()

  bars <- await(bars_promise)
  clock <- await(clock_promise)

  cat("Got", nrow(bars), "bars for AAPL\n")
  cat("Market is", if (clock$is_open) "OPEN" else "CLOSED", "\n")
  return(invisible(NULL))
})

main()
while (!loop_empty()) {
  run_now()
}
```

## Parallel Requests

Fire off multiple requests simultaneously and collect results:

``` r

library(alpaca)
library(coro)
library(later)

md <- AlpacaMarketData$new(async = TRUE)
symbols <- c("AAPL", "MSFT", "GOOGL", "AMZN", "TSLA")

# Launch all requests in parallel, then await each in sequence inside
# a single async() block. The requests run concurrently — only the
# awaits are sequential.
collect <- async(function() {
  bar_promises <- lapply(symbols, function(sym) {
    return(md$get_latest_bar(symbol = sym))
  })
  results <- vector("list", length(bar_promises))
  for (i in seq_along(bar_promises)) {
    results[[i]] <- await(bar_promises[[i]])
  }
  return(data.table::rbindlist(results, fill = TRUE))
})

result_promise <- collect()
while (!later::loop_empty()) {
  later::run_now()
}
print(await(result_promise))
```

## Async Trading

Place orders asynchronously and monitor their status:

``` r

library(alpaca)
library(coro)
library(later)

trading <- AlpacaTrading$new(async = TRUE)
acct <- AlpacaAccount$new(async = TRUE)

main <- async(function() {
  # Place order
  order <- await(trading$add_order(
    symbol = "AAPL",
    side = "buy",
    type = "limit",
    time_in_force = "day",
    qty = 1,
    limit_price = 150
  ))
  cat("Order placed:", order$id, "\n")

  # Check account
  info <- await(acct$get_account())
  cat("Buying power:", info$buying_power, "\n")

  # Check positions
  positions <- await(acct$get_positions())
  cat("Open positions:", nrow(positions), "\n")
  return(invisible(NULL))
})

main()
while (!loop_empty()) {
  run_now()
}
```

## Async in Shiny

In Shiny, the event loop runs automatically. Just return promises from
reactive expressions:

``` r

library(shiny)
library(alpaca)

md <- AlpacaMarketData$new(async = TRUE)

server <- function(input, output, session) {
  bars <- reactive({
    return(md$get_bars(
      symbol = input$symbol,
      timeframe = "1Day",
      start = input$start,
      end = input$end
    ))
  })

  output$chart <- renderPlot({
    coro::async(function() {
      data <- await(bars())
      return(plot(
        data$t,
        data$c,
        type = "l",
        main = paste(input$symbol, "Close Price"),
        xlab = "Date",
        ylab = "Price"
      ))
    })()
  })
  return(invisible(NULL))
}
```

## Error Handling

Use [`tryCatch()`](https://rdrr.io/r/base/conditions.html) inside an
[`async()`](https://coro.r-lib.org/reference/async.html) block —
[`await()`](https://coro.r-lib.org/reference/async.html) re-throws the
promise rejection as a normal R error, so the JS try/catch pattern works
directly:

``` r

library(alpaca)
library(coro)
library(later)

md <- AlpacaMarketData$new(async = TRUE)

main <- async(function() {
  return(tryCatch(
    {
      data <- await(md$get_bars(
        symbol = "INVALID_SYMBOL",
        timeframe = "1Day"
      ))
      cat("Got", nrow(data), "bars\n")
    },
    error = function(err) {
      cat("Error:", conditionMessage(err), "\n")
      return(invisible(NULL))
    }
  ))
})

main()
while (!later::loop_empty()) {
  later::run_now()
}
```

## Next Steps

- See
  [`vignette("getting-started")`](https://dereckscompany.github.io/alpaca/articles/getting-started.md)
  for synchronous usage and full API coverage.
- Explore the [Alpaca API documentation](https://docs.alpaca.markets/)
  for endpoint details.
