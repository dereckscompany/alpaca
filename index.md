...
**In plain terms:** Alpaca is an online broker that lets you trade US
stocks and options and pull market data through a web interface instead
of a point-and-click app. This package is the R doorway to that service:
it lets you download price history and live quotes, place and manage buy
and sell orders, and check on your account balance and open positions –
all from R scripts. Every request can run the ordinary way, where your
code waits for the answer, or in the background so your program can
carry on and collect the result later. Answers come back as tidy tables
with clean, readable column names and proper timestamps, ready to feed
straight into analysis. Because it can place real orders against a real
account, it is built to be driven carefully, and Alpaca’s paper-trading
accounts let you test everything safely first.
...
legitimately uses `;` inside some URLs) round-trip via
[`URLdecode()`](https://rdrr.io/r/utils/URLencode.html). The encoding is
**lossless** in both directions: each URL is double-encoded (`%` →
`%25`, then `;` → `%3B`) before joining, so a single
[`URLdecode()`](https://rdrr.io/r/utils/URLencode.html) after splitting
recovers the original string even when the upstream URL already
contained `%3B` or other percent-escapes:
